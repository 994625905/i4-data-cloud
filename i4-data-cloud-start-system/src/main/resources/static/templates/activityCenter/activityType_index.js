    ...
    table.on("toolbar(typeTable)",obj=>{
        if(obj.event == "add"){
            Feng.input("新增类型",value=>{
                ...
                let p = {name:value}
                Request.asyncBody(BasePath+"/activityCenter/activityType/insert",{model:p}).then(res=>{
                    Feng.success("新增成功")
                    refresh()
                })
            })
    ...
    table.on("tool(typeTable)",obj=>{
        if(obj.event == "update"){
            ...
            Feng.input("新增编辑类型",value=>{
                ...
                Request.asyncBody(BasePath+"/activityCenter/activityType/update",{model:p}).then(res=>{
                    Feng.success("编辑成功")
                    refresh()
                })
            },obj.data.name)
        ...
        if(obj.event == "delete"){
            Feng.confirm("确定删除该下午茶吗？",()=>{
                Request.async(BasePath+"/activityCenter/activityType/delete",{id:obj.data.id}).then(res=>{
                    Feng.success("删除成功")
                    refresh()
                })
            })
...
function loadTable(){
    ...
    tableRender = Initlay.initTable("#typeTable",BasePath+"/activityCenter/activityType/loadTable",tabCols,"#toolbar",param,null,null,25)
}
/*************************刷新************************/
function refresh(){
