...
layui.use(["layer","table","form"],()=>{
    ...
    table.on("toolbar(taskTable)",obj=>{
        ...
        if(obj.event == "add"){
            Feng.loadWindow("新增点单任务",BasePath+"/afternoonTea/taskDeploy/addPage")
        }
    })
    ...
    table.on("tool(taskTable)",obj=>{
        if(obj.event == "deploy"){
            Feng.confirm("确定发布吗？",()=>{
                Request.async(BasePath+"/afternoonTea/taskDeploy/deploy",{id:obj.data.id}).then(res=>{
                    Feng.success("发布成功")
                    refresh()
                })
            })
        ...
        if(obj.event == "edit"){
            Feng.loadWindow("编辑点单任务："+obj.data.title,BasePath+"/afternoonTea/taskDeploy/editPage?id="+obj.data.id)
        }
        if(obj.event == "delete"){
            Feng.confirm("确定删除吗？",()=>{
                Request.async(BasePath+"/afternoonTea/taskDeploy/delete",{id:obj.data.id}).then(res=>{
                    Feng.success("删除成功")
                    refresh()
                })
            })
        ...
        if(obj.event == "detail"){
            Feng.loadWindow(obj.data.title+"详情信息",BasePath+"/afternoonTea/taskDeploy/detailPage?id="+obj.data.id)
        }
    })
...
function loadTable(){
    let tabCols = [[
        ...
        {field: "status",title: "状态",width: TABLE_COL_WIDTH.one_Cols(3),templet(d){
            return "<span class='text-success'>未发布</span>";
        }},
        {fixed:"right",title:"操作",align:"center",width: TABLE_COL_WIDTH.tool(3),toolbar:"#operate"},
    ]]
    tableRender = Initlay.initTable("#taskTable",BasePath+"/afternoonTea/taskDeploy/loadTable",tabCols,"#toolbar",param)
}
function refresh(){
    Initlay.reloadTable(tableRender,param)
}