...
function loadTable(){
    let tabCols = [[
        ...
        {field:"teaImage",title:"下午茶图片",align:"center",templet:"#imageUrl"},
...
function loadChartByType(){
    Request.async(BasePath+"/afternoonTea/select/loadChartByType",param).then(res=>{
        InitChart.barAndPie(selectChartByType,res,"按类型")
    })
}
/*************************加载报表************************/
function loadChart(){
    Request.async(BasePath+"/afternoonTea/select/loadChart",param).then(res=>{
