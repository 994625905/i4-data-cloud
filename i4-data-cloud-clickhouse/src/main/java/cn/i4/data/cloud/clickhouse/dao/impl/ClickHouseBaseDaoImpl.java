package cn.i4.data.cloud.clickhouse.dao.impl;

import cn.hutool.core.date.DateUtil;
import cn.i4.data.cloud.clickhouse.annotation.ClickHouseColumn;
import cn.i4.data.cloud.clickhouse.annotation.ClickHouseID;
import cn.i4.data.cloud.clickhouse.annotation.ClickHouseTable;
import cn.i4.data.cloud.clickhouse.config.ClickHouseConfig;
import cn.i4.data.cloud.clickhouse.dao.ClickHouseBaseDao;
import cn.i4.data.cloud.clickhouse.entity.ClickHouseBaseEntity;
import cn.i4.data.cloud.clickhouse.util.ClickHouseBaseUtil;
...
/**
 * dao层实现类，基类，封装单表的增删改查，多表直接使用原生SQL
 * @author wangjc
 * @title: AbstractClickHouseService
...
public class ClickHouseBaseDaoImpl<T extends ClickHouseBaseEntity> implements ClickHouseBaseDao<T> {
    ...
    private Class<T> tClass;// 当前实体对应泛型
    private Field[] allFieldList;// 当前实体属性数组
    private Map<String,Field> allFieldMap;// key-->字段名, value-->属性名
            ...
            for(T t:data){
                ...
                for(Field field:fieldList){
                    ...
                    if(field.getName().indexOf("java.util.Date") != -1){
                        ...
                        if(getValue != null){
                            getValue = DateUtil.format((Date) getValue,columnType);
                        }else{
    ...
    public int selectCount(String sql) {
        sql = "select count(1) as count from ("+sql+") t";//手动构造统计
